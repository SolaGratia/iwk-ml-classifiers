...
class Pegasos(LinearClassifier):
    ...
    def __init__(self, n_steps=1000, L=0.01):
    ...
    def fit(self, X, Y):
        ...
        for t in range(1, self.n_steps):
            ...
            eta = 1 / (self.L * t)
            score = Yn[i] * (self.w.dot(X[i]))
            if score < 1:
                self.w = (1 - eta * self.L) * self.w + (eta * Yn[i]) * X[i]
            else:
                self.w = (1 - eta * self.L) * self.w
...
class SparsePegasos(LinearClassifier):
    ...
    def fit(self, X, Y):
        ...
        for t in range(1, self.n_steps):
            i = random.choice(range(len(X)))
            eta = 1 / (self.L * t)
            score = Yn[i] * sparse_dense_dot(X[i], self.w)
            if score < 1:
                self.w = (1 - eta * self.L) * self.w
                add_sparse_to_dense(X[i], self.w, (eta * Yn[i]))
            else:
                self.w = (1 - eta * self.L) * self.w


class SparsePegasosWithScalingTrick(LinearClassifier):
    ...
    def fit(self, X, Y):
        ...
        for t in range(1, self.n_steps):
            i = random.choice(range(len(X)))
            eta = 1 / (self.L * t)
            score = (a * Yn[i]) * sparse_dense_dot(X[i], self.w)
            if score < 1:
                if t != 1:
                    a = (1 - eta * self.L) * a
                add_sparse_to_dense(X[i], self.w, (eta * Yn[i] / a))
            else:
                a = (1 - eta * self.L) * a
        self.w = a * self.w
    ...
    def fit(self, X, Y):
        ...
        for t in range(1, self.n_steps):
            ...
            if score < 1:
                if t != 1:
                    a = (1 - eta * self.L) * a
                tmp = 1 + math.exp(Yn[i] * sparse_dense_dot(X[i], self.w))
                add_sparse_to_dense(X[i], self.w, (eta * (Yn[i]/tmp) / a))
            else:
                a = (1 - eta * self.L) * a
        self.w = a * self.w
